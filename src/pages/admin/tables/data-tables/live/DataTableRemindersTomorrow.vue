<template>
  <va-card title="On Process Services" class="mb-8">
    <VaDataTable
      ...
      v-if="tomorrowTasks.constructor === Array"
      :fields="fields"
      :items="tomorrowTasks"
      :columns="fields"
      :per-page="perPage"
...
<script setup lang="ts">
  ...
  const { tomorrowTasks }: any = storeToRefs(Content)
  const perPage: any = 5
  const currentPage = ref(1)

  const pages = computed(() => {
    return perPage && perPage !== 0 ? Math.ceil(tomorrowTasks.value.length / perPage) : tomorrowTasks.value.length
  })
