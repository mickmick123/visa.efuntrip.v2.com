<template>
  ...
  <va-card title="On Process Services" class="mb-8">
    ...
    >
      ...
      <template #cell(client)="{ rowIndex }">{{ `${onProcessPageServices[rowIndex].client.first_name} ${onProcessPageServices[rowIndex].client.last_name}` }}</template>
      <template #cell(type)="{ rowIndex }">{{ `${onProcessPageServices[rowIndex].client.groups.length === 0 ? 'Individual' : onProcessPageServices[rowIndex].client.groups[0].name}` }}</template>
...
<script setup lang="ts">
  ...
  Content.getOnProcessServices(100000, page.value, input.value)
  ...
  const pages = computed(() => {
    return per_page.value && per_page.value !== 0
      ? Math.ceil(onProcessPageServices.value.length / per_page.value)
      : onProcessPageServices.value.length
  })

  const filteredCount = ref(onProcessPageServices.value.length)
  ...
  watch(onProcessPageServices, async (newValue) => {
