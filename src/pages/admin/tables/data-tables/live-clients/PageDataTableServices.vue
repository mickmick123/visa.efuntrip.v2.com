<template>
    ...
    <va-card title="Contact Types" class="mb-8">
        <VaDataTable :row-bind="getRowBind" sticky-header class="custom-table va-table--striped" v-if="clientServices.constructor === Array"
            :fields="fields" :items="clientServices" :columns="fields" :per-page="per_page" :current-page="currentPage">
            <template #cell(charge)="{ value }">
                - {{ value }}
            </template>
        ...
        <va-card title="filter" class="bg-white p-5 w-[100%]">
            <VaAlert v-if="showPayment" class="!mt-2 !mb-5" color="info" outline>
                <VaForm hideErrorMessages ref="paymentRef" class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-6 gap-3">
                    <VaInput class="col-span-2" inner-label
                        :rules="[(value: any) => (value && value.length > 0) || 'This field is required']"
                        v-model="form.amount" label="Amount" placeholder="0.00" />
                    <VaSelect class="col-span-2" v-model="form.payment_mode" :options="payment_options"
                        placeholder="Select an mode" :rules="[(v: any) => v || 'Field is required']" />
                    <VaButton :disabled="form.amount === '' || form.payment_mode === ''" @click="submit()" size="medium"
                        color="primary">
                        Confirm
                    </VaButton>
                    <VaButton @click="cancelPayment()" size="medium" color="danger">
                        Cancel
                    </VaButton>
                </VaForm>
...
import { addContactType, deleteContactType, getClientProfile, payClientService } from '../../../../../services/request'
