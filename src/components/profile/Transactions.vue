      ...
      <VaCardContent>
        <VaForm ref="formRef">
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 items-start">
            ...
            <VaSelect
              v-if="form.storage.value === 'Alipay'"
              v-model="form.bank"
              :options="alipayList"
              :rules="[(v: any) => v || 'Field is required']"
              label="Storage"
            ...
            <VaInput
              v-if="form.fundType.value === 'Deposit'"
              v-model="form.amount"
              placeholder="0"
              label="Amount"
            ...
            <VaSelect
              v-if="form.fundType.value === 'Balance Transfer'"
            ...
            <VaSelect
              v-if="form.fundType.value === 'Balance Transfer' && form.transfer_to.value === 'Client'"
              v-model="form.selected_client"
              :options="clientList"
              :rules="[(v: any) => v || 'Field is required']"
              label="Client"
              placeholder="Search for client id or client name"
              searchable
              @update:search="(val: any) => getClients(val)"
            />

            <VaSelect
              v-if="form.fundType.value === 'Balance Transfer' && form.transfer_to.value === 'Group'"
          ...
          <div class="mt-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <VaButton class="col-start-4" :disabled="!isValid" @click="validate() && submit()">
              Confirm Transaction
            </VaButton>
          </div>
        </VaForm>
