<template>
  <div v-if="!isLoading" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-start !w-full">
    <div
      v-if="!selectedPackage && !packages.length"
      class="col-start-4 flex w-full h-[300px] justify-center items-center"
    >
      <VaButton class="" @click="() => generatePackage()"> Create New Package </VaButton>
    </div>
    <div
      v-if="!selectedPackage && packages.length"
      class="mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 items-start col-span-4"
    >
      <VaButton class="col-start-3 mb-10" @click="() => generatePackage()"> Create New Package </VaButton>
    </div>
    ...
    >
      <VaBadge
        ...
        overlap
        :text="moment(pk.created_at).format('LL')"
        color="#34d2eb"
        ...
        >
          ...
          <VaCardContent>
            <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-1 items-start">
              ...
              <h2 class="col-span-2 mb-3">
                Package Cost:
                <b
                  ><i>{{ pk.package_cost }}</i></b
                >
              </h2>
            </div>
          </VaCardContent>
    ...
    <div v-if="selectedPackage" class="col-start-1 mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 col-span-2">
      <VaButton icon="clear" color="danger" class="mr-3 mb-2" @click="() => resetSelected()"> Go back </VaButton>
      <VaButton
        v-if="!isAddService"
        icon="add"
        color="primary"
        class="mr-3 mb-2 col-span-2"
    ...
    <div v-if="selectedPackage && isAddService" class="col-span-4">
      <VaForm ref="formRef" class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-6 gap-4 items-start">
        ...
        />
        <VaInput
          v-if="form.serviceSelected.length == 2 && !form.serviceSelected[0]"
          v-model="form.serviceSelected[1].charge"
          class="col-start-1"
          inner-label
          readonly
          label="Total Price"
        ...
        />
        <VaInput
          v-if="form.serviceSelected.length == 1"
          v-model="form.serviceSelected[0].charge"
          class="col-start-1"
          inner-label
          readonly
          label="Total Price"
          placeholder="Total"
        />
        <VaButton :disabled="!isValid" size="medium" color="primary" @click="validate() && submit()">
          Save Service
        </VaButton>
        <VaButton size="medium" color="danger" @click="hideAddService()"> Cancel </VaButton>
      </VaForm>
    </div>
    <div v-if="selectedPackage && isShowServices" class="col-span-4">
