                    ...
                    <template #start="{ containerSize }">
                        <div class="p-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 items-end">
                            <div class="col-span-3">
                                <VaFileUpload file-types="jpg,png" type="single" v-model="img" undo :undo-duration="1000"
                                    undo-button-text="Delete" deleted-file-message="File Deleted" />
                            </div>
                        </div>
                    </template>
                    <template #end="{ containerSize }">
                        <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 gap-4 items-end p-5">
                            <VaSelect class="mb-6" v-if="img" v-model="docValue" label="*Document Type"
                                :options="documentTypes" searchable highlight-matched-text />
                            <va-date-input v-if="img" class=" mb-6" :format="formatFn"
                                @click:day="(d: any) => dtSelected('issued_on', d)" label="*Issued on"
                                placeholder="YYYY-MM-DD" />
                            <va-date-input v-if="img" class=" mb-6" :format="formatFn"
                                @click:day="(d: any) => dtSelected('expired_on', d)" label="Expired on"
                                placeholder="YYYY-MM-DD" />
